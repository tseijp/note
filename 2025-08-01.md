...
you write in their proprietary TSL (Three.js Shading Language).
I had wanted to work with WebGPU but lacked time.
...
When I suddenly had time available,
I started studying WebGPU.
...
### Compute Shader Support with Three.js Shader Language Compatible Output to Both GLSL and WGSL
...
### SSH from Smartphone to PC's DevContainer and Operate Claude Code via tmux

PC and smartphone are configured to be on the same VPN using [Tailscale](https://github.com/tailscale/tailscale).
PC's development environment runs containers using VSCode's [devcontainer](https://github.com/microsoft/vscode-dev-containers).
From the smartphone,
...
### Having Claude Code Output Implementation Plans as Requirements Definition → Comment Technical Specifications
...
### TypeScript Shader Language That Converts TypeScript to GLSL and WGSL for Execution

It supports basic GLSL syntax,
...
> [![Screenshot of edge detection demo](https://r.tsei.jp/note/2025-08-01/20250801_glre1.png)](https://glre.dev/guide/techniques/images-textures)
>
...
### Implementing Custom Intermediate Language Compute Shaders That Work on Both WebGL and WebGPU
...
### WebGL Fallback Implementation Running Same Code compute shader
...
Code like the above is generated from the following code.
...
To convert the above fragment function to `fn fragment(uv: vec2f) -> vec4f` in WGSL,
